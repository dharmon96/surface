...
if (!cmd || !file) { console.error("usage: boutkit build|cues <show.json> [outDir] [--mode bridge|direct]"); process.exit(1); }
...
if (cmd === "cues") {
  ...
  const files = await buildBundle(doc, cues, { outDir, companion: { mode: flag("mode", "bridge") as any, boutkitHost: flag("host", "127.0.0.1:8090") } });
